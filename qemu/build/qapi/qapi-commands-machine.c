...
static void qmp_marshal_output_HvBalloonInfo(HvBalloonInfo *ret_in,
                                QObject **ret_out, Error **errp)
{
    Visitor *v;

    v = qobject_output_visitor_new_qmp(ret_out);
    if (visit_type_HvBalloonInfo(v, "unused", &ret_in, errp)) {
        visit_complete(v, ret_out);
    }
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_HvBalloonInfo(v, "unused", &ret_in, NULL);
    visit_free(v);
}
...
void qmp_marshal_query_hv_balloon_status_report(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_HvBalloonInfo(retval, ret, errp);
